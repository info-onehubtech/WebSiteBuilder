    ...
    axios.get(`${API_URL}/admin/templates`)
      .then(res => {
        ...
        if (res.data.templates && res.data.templates.length > 0) {
          setFormData(f => ({ ...f, templateId: res.data.templates[0].folderName }));
        }
      })
      .catch(err => {
        setAdminTemplates([]);
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            ...
            <input type="text" name="siteType" value={formData.siteType} onChange={handleChange} placeholder="e.g. Portfolio, Blog, Business" style={{
              padding: '0.7rem', borderRadius: '0.7rem', border: '1px solid #d0d7de', fontSize: '1rem',
            }} />
            <label style={{
              fontWeight: 'bold',
            ...
            <select name="templateId" value={formData.templateId} onChange={handleChange} style={{
              ...
              {adminTemplates.length === 0 ? (
                ...
                adminTemplates.map(t => (
                  <option key={t.folderName} value={t.folderName}>{t.folderName}</option>
                ))
              )}
            </select>
            {formData.templateId && (
